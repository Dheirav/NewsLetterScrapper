...
Pipeline checkpointing — what it actually does:
    Every durable step writes a row to pipeline_runs, but only STEP 5
    (embedding) reads that table and skips itself. Steps 8, 14 and the email
    step record their completion and nothing consults it. Their work is cheap
    and idempotent, so re-running them on a restart costs little.

    Step 9 (knowledge generation) is the expensive one and is NOT protected by
    pipeline_runs. It has its own guard — it skips clusters that already have a
    saved KnowledgeStoryORM row.

    KNOWN GAP: that guard does not survive a restart. Cluster IDs are fresh
    uuid4 values generated by clusterer.py on every run, so after a crash the
    re-clustered articles carry new IDs that match nothing in knowledge_stories
    and every story is generated again. Making resume genuinely work needs
    either content-derived cluster IDs or reloading the previous run's clusters
    from the database instead of re-clustering.
"""
import argparse
import asyncio
import logging
...
async def run(mode: str = "concise", test: bool = False) -> None:
    # The one and only clock read for this run. Everything downstream — cluster
    # dates, story dates, checkpoint rows, the newsletter, the dead-letter queue
    # — is stamped from this value and never re-reads date.today(). A run that
    # starts at 23:50 and finishes after midnight therefore lands entirely on
    # one date instead of being split across two.
    today = date.today()
    log.info("\n%s", BANNER)
    log.info(
    ...
    async with get_session() as session:
        ...
        # -- 5b. Drop republished stories --------------------------------------
        # Runs here rather than with the other dedup stages because it compares
        # embeddings, which do not exist until step 5. Catches the same story
        # reappearing under a new URL, which URL and title dedup cannot see.
        async with _step(5, "Removing republished stories (semantic dedup)"):
            before = len(embedded)
            embedded = await mark_semantic_duplicates(embedded, session)
            if before != len(embedded):
                log.info(
                    "%d republished articles excluded from clustering",
                    before - len(embedded),
                )
        ...
        async with _step(9, "Generating deep knowledge stories"):
            stories = await generate_knowledge_stories(
                clusters, session, run_date=today, max_concurrent=6, mode=mode
            )
