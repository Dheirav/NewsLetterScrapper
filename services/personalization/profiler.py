...
async def build_profile(session: AsyncSession) -> UserProfile:
    ...
    All DB work is done in a single JOIN query (events → clusters → articles)
    instead of the previous three sequential round-trips, reducing DB latency
    on the profile-build step by ~60%.
    """
    cutoff = date.today() - timedelta(days=settings.profile_window_days)

    # ── Single JOIN: events + clusters + articles ─────────────────────────
    # Returns (event, cluster_topic_label, article_source) rows in one query.
    stmt = (
        select(
            ReadingEventORM.topic_slug,
            ReadingEventORM.time_spent_seconds,
            ReadingEventORM.scroll_percent,
            StoryClusterORM.topic_label,
            ArticleORM.source,
        )
        .select_from(ReadingEventORM)
        .outerjoin(
            StoryClusterORM,
        ...
        .outerjoin(
            ArticleORM,
            ArticleORM.cluster_id == StoryClusterORM.cluster_id,
        )
        .where(ReadingEventORM.event_date >= cutoff)
    )
    ...
    seen_event_slugs: set[tuple] = set()   # deduplicate event rows (cartesian)
    all_scrolls: list[float] = []

    for topic_slug, time_sec, scroll_pct, cluster_label, article_source in rows:
        # Each event row is repeated once per matching article (JOIN fanout).
        # Accumulate topic metrics only once per unique (slug, time, scroll).
        key = (topic_slug, time_sec, scroll_pct)
        if key not in seen_event_slugs:
            seen_event_slugs.add(key)
            topic_time[topic_slug].append(float(time_sec))
            topic_scroll[topic_slug].append(float(scroll_pct))
            all_scrolls.append(float(scroll_pct))

        # Source affinity — accumulate even duplicate rows (more articles = higher signal)
        if article_source and cluster_label:
            cluster_slug = slugify(cluster_label)
            if cluster_slug == topic_slug:
                source_scores[article_source].append(float(scroll_pct) / 100)
    ...
    # ── Preferred depth ────────────────────────────────────────────────────
    preferred_depth = sum(all_scrolls) / len(all_scrolls) if all_scrolls else 50.0
